angular.module('nagifront')
  .directive('serviceTrend', ['d3', '$http', '$interval', 'djangoUrl', function(d3, $http, $interval, djangoUrl){
    return {
      restrict: 'EA',
      scope: true,
      template: '<h3>{{ data.service.display_name }} <select ng-model="time"'
      ...
      link: function(scope, element, attrs){
        ...
        getData = function(){
          $http.get(djangoUrl.reverse('hosts-services-id-trend', { service_id: attrs.serviceId })).then(function(response){
        ...
        scope.render = function(data){
          ...
          var xLabel = [{s: 1, n: 'Unknown'}, {s: 2, n: 'Critical'}, {s: 3, n: 'Warning'}, {s: 4, n: 'Up'}]
...
angular.module('nagifront')
  .directive('serviceTrendHost', ['d3', '$http', 'djangoUrl', function(d3, $http, djangoUrl){
    return {
      restrict: 'EA',
      scope: true,
      template: '<h3>서비스 트렌드 - <select name="host-id" ng-model="host_id"'
        + 'ng-options="host_id.host_object_id as host_id.name for host_id in host_ids" ng-disabled="!is_modify_setting"></select></h3>'
        + '<div class="charts" ng-if="!is_modify_setting">'
          + '<scrollable always-visible="true">'
            + '<div ng-repeat="member in members" service-trend service_id="{{ member }}">'
          + '</scrollable>'
        + '</div>',
      link: function(scope, element, attrs){
